...
function SurahDetail() {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 pb-24"> {/* Added padding to the bottom */}
      {loading ? (
        <p className="text-center text-lg text-gray-600">Memuat...</p>
      ) : surah ? (
        <div className="bg-white shadow-lg rounded-lg p-6">
          <h1 className="text-3xl font-bold text-center text-gray-800 mb-4">{surah.nama_latin}</h1>
          <p className="text-gray-600 mb-4 italic text-center">{surah.deskripsi}</p>
          ...
          <div className="mb-4">
            <select
              ...
              className="border border-gray-300 rounded-md p-2 w-full focus:outline-none focus:ring-2 focus:ring-green-500 transition duration-200"
            >
          ...
          <ul className="space-y-4">
            {displayedAyahs && displayedAyahs.length > 0 ? (
              displayedAyahs.map((ayah) => (
                <li key={ayah.id} className="p-4 bg-gray-50 rounded-lg shadow border border-gray-200 flex items-start">
                  <span className="text-lg font-semibold text-gray-900 mr-4">{ayah.nomor}.</span>
              ...
              <li className="p-4 bg-gray-50 rounded-lg shadow border border-gray-200 text-center text-gray-700">
